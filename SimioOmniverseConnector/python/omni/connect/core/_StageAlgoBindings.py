...
from ._omni_connect_core import configureStage, logChannel, setLayerAuthoringMetadata

# Attempt to import omni.log but handle failures as logging is not a hard requirement
# This allows omni.connect.core to successfully be imported in python environments that do not "carb_sdk_plugins" in the PYTHON_PATH
try:
...
def createStage(identifier: str, defaultPrimName: str, upAxis: str, linearUnits: float, fileFormatArgs: Optional[dict] = None) -> Optional[Usd.Stage]:
    """
    ...
    """
    # This function should mimic the behavior of the C++ function `omni::connect::core::createStage`.
    ...
    if not identifier or not Usd.Stage.IsSupportedFile(identifier):
        if omni_log is not None:
            omni_log.warn(f'Unable to create UsdStage at "{identifier}" due to an invalid identifier', channel=logChannel())
        return None
    ...
    setLayerAuthoringMetadata(stage.GetRootLayer())
    ...
    if not configureStage(stage, defaultPrimName, upAxis, linearUnits):
        return None
